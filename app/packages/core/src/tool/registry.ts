...
const registryLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const settleWith = Effect.fn("ToolRegistry.settle")(function* (input: ExecuteInput, advertised?: object) {
      if (!ResearchPolicy.readOnlyTool(input.call.name))
        return {
          result: {
            type: "error" as const,
            value: `Tool disabled in research mode: ${input.call.name}`,
          },
        }
      const registration =
    ...
    return Service.of({
      ...
      materialize: Effect.fn("ToolRegistry.materialize")(function* (permissions = []) {
        ...
        for (const [name, registration] of registrations)
          if (!ResearchPolicy.readOnlyTool(name) || whollyDisabled(permission(registration.tool, name), permissions))
