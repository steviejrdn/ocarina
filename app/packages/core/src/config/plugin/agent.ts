...
import { ResearchPolicy } from "../../research-policy"
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.agent.transform(
      Effect.fn(function* (draft) {
        ...
        // Configuration is untrusted input in research mode. Append the
        // deny rules after user rules so a wildcard allow cannot re-enable
        // mutation-capable tools.
        for (const current of draft.list())
          draft.update(current.id, (agent) => agent.permissions.push(...ResearchPolicy.permissionRules()))
      }),
    )
  }),
})
