...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["ocarina.json", "ocarina.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".ocarina", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".ocarina")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".ocarina").toReversed()
    ...
    // global config, project files, then `.ocarina` files.
