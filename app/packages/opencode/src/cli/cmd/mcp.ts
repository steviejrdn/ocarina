...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in ocarina.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .ocarina/ subdirectory too)
  const candidates = [path.join(baseDir, "ocarina.json"), path.join(baseDir, "ocarina.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".ocarina", "ocarina.json"), path.join(baseDir, ".ocarina", "ocarina.jsonc"))
  }
  ...
  // Default to ocarina.json if none exist
  return candidates[0]
}
