...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    UI.println()
    UI.println("Starting ocarina...")
    UI.println()

    const ocarinaArgs = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn(["ocarina", ...ocarinaArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`ocarina exited with code ${code}`))
  }),
})
