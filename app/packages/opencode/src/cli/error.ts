...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, ocarina does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`ocarina models\` to list available models`,
      `Or check your config (ocarina.json) provider/model names`,
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`ocarina auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
