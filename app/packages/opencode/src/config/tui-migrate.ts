...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from ocarina.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const ocarina = await ocarinaFiles(input)
  for (const file of ocarina) {
...
async function ocarinaFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "ocarina"),
    ...(await Filesystem.findUp(["ocarina.json", "ocarina.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "ocarina"))
  }
