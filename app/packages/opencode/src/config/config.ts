...
function globalConfigFile() {
  const candidates = ["ocarina.jsonc", "ocarina.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ocarina.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ocarina.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("ocarina", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".ocarina") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["ocarina.json", "ocarina.jsonc"]) {
          ...
          // Auto-discovered plugins under `.ocarina/plugin(s)` are already local files, so ConfigPlugin.load
