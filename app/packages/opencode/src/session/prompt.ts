...
export interface Interface {
  ...
  readonly shell: (input: ShellInput) => Effect.Effect<SessionV1.WithParts, Session.BusyError | ShellDisabledError>
  readonly command: (input: CommandInput) => Effect.Effect<SessionV1.WithParts, Image.Error | CommandDisabledError>
  readonly resolvePromptParts: (template: string) => Effect.Effect<PromptInput["parts"]>
}

export class ShellDisabledError extends Schema.TaggedErrorClass<ShellDisabledError>()("SessionShellDisabledError", {}) {}

export class CommandDisabledError extends Schema.TaggedErrorClass<CommandDisabledError>()(
  "SessionCommandDisabledError",
  {},
) {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            // SessionTools also knows how to materialize MCP and extension
            // tools. The legacy Ocarina runtime must not let those paths reach
            // the provider, even when a plugin or MCP server is configured.
            const tools = Object.fromEntries(
              Object.entries(resolvedTools).filter(([id]) => ToolRegistry.isResearchTool(id)),
            )
    ...
    const shell: (input: ShellInput) => Effect.Effect<SessionV1.WithParts, Session.BusyError | ShellDisabledError> = Effect.fn(
      "SessionPrompt.shell",
    )(function* (input: ShellInput) {
      void input
      return yield* new ShellDisabledError()
    })
