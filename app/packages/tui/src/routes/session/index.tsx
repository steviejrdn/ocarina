...
export function Session() {
  ...
  const sessionCommands = createMemo(() =>
    sessionCommandList().map((command) => {
      assertOcarinaCommandAllowed(command.value)
      return {
    ...
    >
      ...
      <box>
        ...
        <For each={props.sessions}>
          {(item) => (
            <text fg={item.id === props.selected ? theme.text : theme.textMuted} wrapMode="none">
              {item.id === props.selected ? "› " : "  "}
              {Locale.truncate(item.title, 20)}
            </text>
          )}
        </For>
      </box>
          ...
          >
            <text fg={hover() ? theme.text : theme.textMuted} attributes={TextAttributes.BOLD}>
              ...
              <Show when={ctx.showTimestamps()}>
                <span style={{ fg: theme.textMuted, bold: false }}>
                  {" · "}
...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage }) {
  ...
  const researchTool = createMemo(() => ["question", "webfetch", "websearch"].includes(props.part.tool))
  ...
  return (
    <Show when={researchTool() && !shouldHide()}>
      <Switch>
