    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure your Ocarina provider settings to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={() =>
        ({
          opencode: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                OpenCode Zen gives you access to a curated set of research models through a single API
                key.
              </text>
              <text fg={theme.text}>
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure your Ocarina provider settings to use it.`,
          })
          dialog.clear()
          return
        }
