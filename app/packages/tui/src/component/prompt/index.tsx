...
import { assertOcarinaCommandAllowed, isOcarinaSlashCommand, OCARINA_LOCAL_SLASH_COMMANDS } from "../../prompt/commands"
...
export function Prompt(props: PromptProps) {
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      assertOcarinaCommandAllowed(entry.name)
      return { namespace: "palette" as const, ...entry }
    }),
  )
  ...
  async function submitInner() {
    ...
    const typedCommand = trimmed.split(/\s/, 1)[0]?.slice(1)
    if (trimmed.startsWith("/") && typedCommand && !isOcarinaSlashCommand(typedCommand)) {
      await DialogAlert.show(dialog, "Unavailable", "This command is not available in the milestone interface yet.")
      return true
    }
    ...
    if (sessionID == null) {
      ...
      if (res.error) {
        toast.show({
          message: "Creating a session failed. Please try again.",
          variant: "error",
        })
    ...
    const commandName = firstLine.split(" ", 1)[0]?.slice(1)

    if (
      inputText.startsWith("/") &&
      sync.data.command.some((x) => x.name === commandName && x.source !== "mcp" && isOcarinaSlashCommand(x.name))
    ) {
      // Parse command from first line, preserve multi-line content in arguments
      const firstLineEnd = inputText.indexOf("\n")
      const commandLine = firstLineEnd === -1 ? inputText : inputText.slice(0, firstLineEnd)
      const [command, ...firstLineArgs] = commandLine.split(" ")
            ...
            <box flexDirection="row" flexShrink={0} paddingTop={1} gap={1} justifyContent="space-between">
              <box flexDirection="row" gap={1}>
                <Show when={local.agent.current()} fallback={<box height={1} />}>
                  {(agent) => (
                    <>
                      <text fg={fadeColor(highlight(), agentMetaAlpha())}>{agent().name}</text>
