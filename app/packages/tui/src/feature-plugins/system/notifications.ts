...
const tui: TuiPlugin = async (api) => {
  ...
  api.event.on("session.status", (event) => {
    ...
    notify(api, sessionID, `${BRAND.session} complete`, session?.parentID ? "subagent_done" : "done")
  })
